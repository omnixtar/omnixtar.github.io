function log(s) {
 let textNode = document.createTextNode(s); document.body.appendChild(textNode); 
}
// alert(prompt("start libphosm:"));
// log("start libphosm1 ");

function f_log() {
 let textNode = document.createTextNode(s.pop()); document.body.appendChild(textNode); 
}
...
import {
    ...
    f_app,
    f_dom,
    f_file, f_load, f_blob, f_download, f_sse
} from './libdom.js';

// log("<br>  20 import libphosm1 ");
...
// log("<br>  40 rsa import libphosm1 ");
...
// i2p router cannot handle domain name?
...
function Phos() {
    ...
    async function f_awt() {
        ...
        if (f0[L - 1] == ':')
            ...
            f1 = f0;
        alert('  awt: ' + $a + '  xk ' + S0.xk + '  cda ' + S0.cda.toString() + '  CDW ' + S0.$CDW[S0.cda[0]][S0.cda[1] + 1] + '  ' + f1);
    ...
    function f_ssl() { // stack slice
        var j = s.pop();
        var i = s.pop();
        console.log(s.length,i,j)
        s.push(s.slice(s.length-i, s.length-i+j));
    } 
    ...
    function f_ms() { // merge string on stack
        var i;
        var N = S.pop();
        var s = '';
        var c;
    ...
    var f_a2t=function(){
      var u=''; var a=s.pop() 
      a.map(e=>{s.push(e); f('td tn: tr tn:'); return s.pop()}).map(e=>{u+=e}) 
      s.push(u); f('table tn:');
    }
    var f_map=function(){
      var u; var a=s.pop() 
    ...
    var f_map1=function(){
      console.log('in map1:')
      var u; var a=s.pop() 
      console.log(window.r); var l=r.length; var w=r[l-2];
      u=a.map(e=>{console.log(e,w);s.push(e);f(w);var p=s.pop();console.log(p);return p;});
      // a.map(e=>{s.push(e); f('td tn: tr tn:'); return s.pop()}).map(e=>{u+=e}) 
      s.push(u); // f('table tn:');
	    window.r.pop() // pop one word for map
      console.log('map1',window.r)
      S0.skip=1;
    }
    function fgl_ce() {
    ...
    function Bs() { // with sid session id
        console.log(arguments);
        $.F('nxhr: auth/phos.php xo: xsrqh:');
        $.F(arguments[0], 'je: xsend:');
    }
    ...
    function f_B() {
        ...
        F('nxhr: auth/phosm2.php xo: xsrqh:');
        var t = S.length - 1;
        var ajax = S[t];
    ...
    async function f_AUTH_DBG() { // Debug I2P 2024-11-21
        var NN = S.pop();
        var K = S.pop(); // key pair, K.privateKey to decrypt secret phrase ciphertext after ajax
        console.log(arguments);
        var pbk = S[S.length - 1]; // s_pbk sent to server to encrypt secret phrase
        console.log(pbk);
        $.F('F_AUTH_DBG swap:'); // 2024-11-21 preserve pbk
        $.F('nxhr: auth/phos.php xo: xsrqh:');
        var t = S.length - 1;
        var ajax = S[t];
        S[t].onreadystatechange = async function () {
            if (ajax.readyState == 4 && ajax.status == 200) {
                console.log(ajax.responseText);
                S.push('    F_AUTH_DBG '+ajax.responseText);
                var E1 = ajax.responseText;
                var D1 = await window.crypto.subtle.decrypt({ name: 'RSA-OAEP' }, K.privateKey, str2ab(atob(E1)));
                console.log(ab2str(D1));
                S.push('   D1 '+   ab2str(D1)  );
    ...
    async function f_AUTH_DBG1() { // Debug I2P 2024-11-21
        var NN = S.pop();
        var K = S.pop(); // key pair, K.privateKey to decrypt secret phrase ciphertext after ajax
        console.log(arguments);
        var pbk = S[S.length - 1]; // s_pbk sent to server to encrypt secret phrase
        console.log(pbk);
        $.F('F_AUTH_DBG1 swap:'); // 2024-11-21 preserve pbk
        $.F('nxhr: auth/phos.php xo: xsrqh:');
        var t = S.length - 1;
        var ajax = S[t];
        S[t].onreadystatechange = async function () {
            if (ajax.readyState == 4 && ajax.status == 200) {
                console.log(ajax.responseText);
                S.push('    F_AUTH_DBG1 '+ajax.responseText);
                var E1 = ajax.responseText;
                var D1 = await window.crypto.subtle.decrypt({ name: 'RSA-OAEP' }, K.privateKey, str2ab(atob(E1)));
                console.log(ab2str(D1));
                S.push('   D1 '+   ab2str(D1)  );
                var a_csv = ': auth_csv dts: PASS ka: AUTH uss: ' + btoa(pbk) + ' PBK ka: AUTH uss: ' + NN + ' NN ka: AUTH uss: ; ';
                // B(a_csv + ab2str(D1) + ' AUTH gsv: SP i: cmp: 0 ifeq: auth_csv b chromy ' + btoa(pbk) + ' ON ECHO bv: ec: ec: 202411 sje: tmp202411 7581 wdbg:');
                B(a_csv + ab2str(D1) + ' AUTH gsv: SP i: SESSION: sje: tmp202411 7581 wdbg: . cmp: 0 ifeq: auth_csv b chromy ')
                
                // do not use A_AUTH1 etc as it will interfere with other AJAX functions
                // B_* back end function; A_* AJAX functions
                $.F(': B_TEST 111 5555 + s: ; : A_TEST B: B_TEST F_O ; A_TEST');

            }
        };
        $.F(btoa(pbk) + ' req_auth_sc SESSION: sje: tmp_session_2024_11 7581 wdbg: .', 'je: xsend:');
    }
    ...
    function fgl_wo() { // window.open
        window.open(S.pop());
    }
    
    function fgl_qsa() { // querySelectorAll
        S.push(document.querySelectorAll(S.pop()));
    }
    function fgl_qs() { // querySelector
        S.push(document.querySelector(S.pop()));
    }
...
//    this.FGL = async function ($a) {
    this.FGL = function ($a) {
        ...
        // Used for map, if, control etc. add r to S0
        window.r = $a.toReversed();
        var r = window.r;
        if (S0.CDL == 0) {
            $CDW.phos = $a;
            S0.CDW[0] = [
            ...
            S0.CDN = S0.CDL - 1;      // CDL=colon definition level; CDN=CD index
            S0.cda = S0.CDW[S0.CDN];  // cda=current list of words, in CDW stack, CDN index
        }
        var $SL = end(end(S0.CDW));
        ...
        do {
            ...
            if (in_array($v, array_keys($CDW))) {
                ...
                if (end($WA) == ';')
                    array_pop($WA);
                S0.CDW.push([   // if word is colon defintion, push to CDW stack
                    $v,
                    $vk,
                    {}
                ]);
                console.log(986, 'before FGLA', JSON.stringify(S0.CDW));
                // await FGLA($WA);
                FGLA($WA);      // execute
                S0.CDW.pop();   // end colon definition execution, pop CDW stack
                console.log(990, 'after FGLA', JSON.stringify(S0.CDW));
                S0.cda = end(S0.CDW);
            } else if ($v == '>:' || $v == '<:') {
                S.push($v);
            } else {
                ...
                if ($v[0] == ':' && $l == 1) {
                    ...
                    if (ord($v) == 0);
                    else if ($v[0] == '_') { // disable all words starting with _ ??
                        if ($v == '_')
                            S.push($v);
                    } else {
                        if ($v == '.s') {
            ...
            if ($xk >= $xl)
                break;
            console.log('libphosm1 r',window.r);    
            window.r.pop();
        } while ($vk < $xl);
    };
    var FGL = this.FGL;
...
//if (typeof window.S[0].dlb==="undefined") {
...
// add post processing functions before F_O output to DOM
// define F_P in console or elsewhere?
// B: only accepts 2 args B_F back end function and F_O output function
// so must modifiy arg2 : F_P do something F_O
M.F(': AJAXP B: B_F F_P ;');

// Decentralized User Centric DU = Duniix

// colon definition AUTH button
// M.F(': B_AUTH '+ localStorage.getItem("sid") +' ssid: SESSION: AUTH i: s: ; : A_AUTH B: B_AUTH F_O ;');
...
// synchronise oxm.php on Monocles mobile browser
// alert(prompt("before gui_dsc:"));

// path is always relative to home, no access to .. parent!!
include("./Phos/gui_dsc.js");

// alert(prompt("before init:"));

// current directory is based on home page oxm.php
include("./init.js");

// alert(prompt("after init:"));
